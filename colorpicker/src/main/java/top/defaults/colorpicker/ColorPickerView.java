...
public class ColorPickerView extends LinearLayout implements ColorObservable {
    public static final String TAG = ColorPickerView.class.getName();

    public ColorWheelView colorWheelView;
    public BrightnessSliderView brightnessSliderView;
    ...
    public ColorPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        setPadding(margin, margin, margin, margin);//去掉padding
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TAG, String.format("maxWidth: %d, maxHeight: %d", maxWidth, maxHeight));
        }
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TAG, String.format("desiredWidth: %d", desiredWidth));
        }
    ...
    public void setEnabledBrightness(boolean enable) {
        if (enable) {
            if (brightnessSliderView == null) {
                ...
                params.topMargin = sliderMargin / 3;//sliderMargin;
